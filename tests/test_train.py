...
def test_curated_에서는_셋_다_돈다():
    ...
    names = {s.name for s in BASELINES if s.runs_on("curated")}
...
def test_all_에서는_logreg_를_뺀다():
    ...
    names = {s.name for s in BASELINES if s.runs_on("all")}
...
def test_바닥을_재는_모델은_모든_집합에서_돈다():
    ...
    assert dummy.runs_on("curated") and dummy.runs_on("all")